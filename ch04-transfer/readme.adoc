...
このスクリプトは第一引数に宛先アドレスを第二引数に送信するモザイク（ `cat.currency` ）の相対量を指定します。

ここでは `bob` のアドレスへ `100 cat.currency` 送ります。
...
`100 cat.currency` が到着していることが確認できました。
...
`NetworkCurrencyMosaic` とは基軸通貨である `cat.currency` に特化したクラスです。
