# 参考サイト・プロジェクト

公式・有志の情報が集まっているサイトのリンク集です。

## 公式ドキュメント

- [NEM Developer Center](https://nemtech.github.io/ja/)
- [NEM Foundation Roadmap \| NEM Documentation](https://docs.nem.io/en/roadmap)
- [NEM Symbol Testnet \| NEM Documentation](https://docs.nem.io/en/testnet)


## APIドキュメント

- [エンドポイント — NEM Developer Center](https://nemtech.github.io/ja/endpoints.html)
    - https://nemtech.github.io/_downloads/openapi3.yaml
- [ガイド — NEM Developer Center](https://nemtech.github.io/ja/guides)


## 有志による情報

- [NEMTionary](https://scrapbox.io/NEMTionary/)
- [catapultに関する96件の記事 \- Qiita](https://qiita.com/tags/catapult)
- [my\-catapult\-docs/reverse\_lookup\.md at master · mijinc0/my\-catapult\-docs](https://github.com/mijinc0/my-catapult-docs/blob/master/reverse_lookup.md)


### 抜粋記事

- [Symbolで変わる現行NEMの機能 \- Qiita](https://qiita.com/daoka/items/8a4c691b761f229a47e5)
- [ざっくり分かるNEM2\(Symbol\) 〜マルチレベルマルチシグネチャ〜 \- Qiita](https://qiita.com/daoka/items/90964c84eb103fcc519b)
- [カタパルトで使われるデータをさくっと変換するWebサイトつくりました \- Qiita](https://qiita.com/planethouki/items/5bbd192d07e4faee5096)
- [プライベートチェーンのカタパルトで秒間4000トランザクションを目指してみる \- Qiita](https://qiita.com/planethouki/items/9733aa83096a988ee57a)
- [カタパルトで秒間4000内部トランザクションを目指してみる \- Qiita](https://qiita.com/planethouki/items/eb19ed496aa8b6d5533a)
- [お財布に優しいSymbol 4000TPSチャレンジ \(その1\) \- Qiita](https://qiita.com/hide825jp/items/271bd8d5159e24dae2ee)
- [複数のSymbolノードで4,000TPSに挑戦してみた\(瞬間4,000TPS\) \- Qiita](https://qiita.com/44uk_i3/items/020007b738ac85af6eb2)
- [nem catapult secret lock/proof tx 異なる２つのMIJIN\_TESTチェーン間でのアトミックスワップ \- Qiita](https://qiita.com/planethouki/items/3d6c743f58692adf5814)
- [NEM2カタパルトとETHのアトミックスワップ \- Qiita](https://qiita.com/planethouki/items/82e987bd5481c539aaa4)
- [NEM2カタパルトで双方向ペイメントチャネルするには その１ 概要 \- Qiita](https://qiita.com/planethouki/items/c22a21836d913418de82)
- [nem2のサプライチェーンマネジメントケースのワークショップ用プロジェクトをやってみた \- Qiita](https://qiita.com/44uk_i3/items/b9c4fc5751d93bf50a3d)


## 実装参考・サンプルコード

- [evias/nem2\-sandbox: a simple sandbox to work with nem2\-sdk\-typescript\-javascript](https://github.com/evias/nem2-sandbox)
- [planethouki/nem2\-tiny\-wallet](https://github.com/planethouki/nem2-tiny-wallet)
- [planethouki/nem2\-tiny\-faucet](https://github.com/planethouki/nem2-tiny-faucet)
- [planethouki/nem2\-helper\-web: NEM2 サクサク データ変換Web](https://github.com/planethouki/nem2-helper-web)
- [planethouki/nem2\-unsafe\-wallet\-nuxt: NEM2 テスト用ウォレット](https://github.com/planethouki/nem2-unsafe-wallet-nuxt)
- [Matsushin/nem2\-sdk\-wallet\-vue\-sample](https://github.com/Matsushin/nem2-sdk-wallet-vue-sample)
- [44uk/nem2\-faucet: Faucet application for nem2 \(catapult\)](https://github.com/44uk/nem2-faucet)
- [44uk/nem2\-dev\-ui: nem2\(catapult\) development Utility Interface](https://github.com/44uk/nem2-dev-ui)


## 参考プロジェクト

- [nemfoundation/nem2\-wallet\-browserextension: nem2 browser wallet\. Experimental, use in your own risk\.](https://github.com/nemfoundation/nem2-wallet-browserextension)
- [nemfoundation/nem2\-hd\-wallets: Experimental library to handle hyper deterministic wallets for Symbol](https://github.com/nemfoundation/nem2-hd-wallets)
- [nemtech/nem2\-camel: A component to turn the asynchronous transaction announcement into synchronous\.](https://github.com/nemtech/nem2-camel)
- [NemTechCN/Nem2\-Dapp\-Plugin](https://github.com/NemTechCN/Nem2-Dapp-Plugin)
- [sleepyOwl14/NEM2\-GraphQL: GraphQL for NEM2 MongoDB](https://github.com/sleepyOwl14/NEM2-GraphQL)
- [empleos/catapult\-server\-docs: Documentation on NEM 2 Symbol server](https://github.com/empleos/catapult-server-docs)
- [CrackTheCode016/cat\-config\-scripts: A series of shell scripts used for configuring a Symbol node](https://github.com/CrackTheCode016/cat-config-scripts)
